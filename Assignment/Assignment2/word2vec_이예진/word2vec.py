...
def naiveSoftmaxLossAndGradient(
    ...
    # # handout 과 기호 통일
    # v_o, u_o, u  = centerWordVec, outsideWordIdx, outsideVectors
    
    # logit = np.matmul(u_o, np.transpose(v_o))     # u_o.T * vc     # shape (V, )
    # y_bar = softmax(logit)
    # yo_bar = y_bar[u_o]
    # loss = -np.log(yo_bar)
    ...
    # V,N = u.shape
    # y = np.zeros(V)
    # y[u_o] = 1    # outside word 벡터 맞는 것에 1 표시

    # # gradient
    # gradCenterVec = np.matmul(y_bar-y, u)   # shape(N, )
    # gradOutsideVecs = np.outer(y_bar-y, centerWordVec)   # shape(V,N)
    ...
    gradCenterVec = np.matmul(y_bar-y, outsideVectors) # shape(N,)
    gradOutsideVecs = np.outer(y_bar-y, centerWordVec) # shape(V,N)
