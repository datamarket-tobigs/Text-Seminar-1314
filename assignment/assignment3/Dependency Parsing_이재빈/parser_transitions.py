...
class PartialParse(object):
    def __init__(self, sentence):
        ...
        self.stack = ["ROOT"]       # dependency parsing 할 때 'root' 넣고 시작
        self.buffer = [word for word in self.sentence]  # buffer에서 stack으로 이동하면서 관계 파악
        self.dependencies = []      # set of arcs (dependency)
    ...
    def parse_step(self, transition):
        ...
        if transition == "S":       # shift : buffer에서 stack 으로 이동
            self.stack.append(self.buffer[0])   # 가장 처음에 있는 거를 stack으로, 끝에 넣고
            self.buffer.pop(0)                  # buffer의 가장 처음꺼 꺼냄
        elif transition == "LA":    # left-arc : 오른쪽에서 (->) 왼쪽으로 dependency가 결정
            self.dependencies.append((self.stack[-1], self.stack[-2]))
            self.stack.pop(-2)
        elif transition == "RA":    # right-arc : 왼쪽에서 (->) 오른쪽으로 dependency 결정
...
def minibatch_parse(sentences, model, batch_size):
    ...
    # dependency parsing 진행
    while len(unfinished_parses) > 0:
        batch = unfinished_parses[:batch_size]      # batch size 만큼 뽑아서 진행
        transitions = model.predict(batch)          # 쌓은 Linear layer로 decision 결정
