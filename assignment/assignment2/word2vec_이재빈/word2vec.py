...
def naiveSoftmaxLossAndGradient(
    ...
    loss  = - np.log(y_hat[outsideWordIdx])     # -log(softmax)

    # Gradient
    d_value = y_hat
    d_value[outsideWordIdx] -= 1    # (y_hat - y), matrix shape (N, 1)
    gradCenterVec   = outsideVectors.T.dot(d_value)     # U x d_value, shape d x 1
    gradOutsideVecs = d_value[:, np.newaxis].dot( np.array([centerWordVec]) )    # (N, 1) x (1, d) -> (N, d)
...
def negSamplingLossAndGradient(
    ...
    o_vector = outsideVectors[outsideWordIdx]             # Uo
    neg_vector = outsideVectors[negSampleWordIndices]     # negative sample

    value_outside  = o_vector.dot(centerWordVec)          # Uo x Vc
    value_negative = neg_vector.dot(centerWordVec)        # neg x Vc

    p_outside  = sigmoid(value_outside)                   # activate function
    p_negative = sigmoid(- value_negative)

    # J(vc, o, U) = -log(sigmoid(u_o * u_c) - sum(log(sigmoid(- u_k * v_c)))
    loss = - (np.log(p_outside) + np.sum(np.log(p_negative)) )

    # 1. delta J / delta vc   2. delta J / delta uo
