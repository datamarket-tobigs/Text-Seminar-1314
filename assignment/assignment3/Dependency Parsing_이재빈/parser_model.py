...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, t):
        ...
        x = self.pretrained_embeddings(t)   # token 의 pre-trained 된 embedding vector 가져오기
        batch_size = t.shape[0]     # tokens (batch_size, n_features) -> t.shape[0] = batch size
        x = x.view(batch_size, -1)  # reshape(batch_size, -1) -> flatten 느낌
    ...
    def forward(self, t):
        ...
        embeddings = self.embedding_lookup(t)   # lookuptable 받아서
        t = self.embed_to_hidden(embeddings)    # layer 거쳐서 embedding 시킴
        relu = nn.ReLU()
        t = relu(t)
        t = self.dropout(t)
