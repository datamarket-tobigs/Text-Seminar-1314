...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == "S": #shift
            self.stack.append(self.buffer.pop(0)) #buffer에서 맨 왼쪽, 즉 0번째 단어를 stack으로 보낸다
        elif transition == "LA": #Left-arc
            head = self.stack[-1] #stack의 마지막 단어를 head로 지정
            dependent = self.stack.pop(-2) #마지막단어 기준 왼쪽 단어 stack에서 제거하면서 dependent로 지정
            self.dependencies.append((head, dependent))
        elif transition == "RA": #Right-arc
            head = self.stack[-2] #head 단어 기준 오른쪽 단어 제거
...
def minibatch_parse(sentences, model, batch_size):
    ...
    #Initialize unfinished_parses as a shallow copy of patial_parses
    unfinished_parses = partial_parses[:]
    
    while True:
        #take the first batch_size parses in unfinished_parses as a mini batch
        mini_batch = unfinished_parses[:batch_size]
        #Use the model to predict the next transition for each partial parse in the mini batch
        transitions = model.predict(mini_batch)
        # Perform a parse_step on each partial parse in the mini batch with its predicted transition
        for partial_parse, transition in zip(mini_batch, transitions):
            partial_parse.parse_step(transition) #위에서 정의한 parse_step 함수를 적용한다
        ...
        if len(unfinished_parses) == 0:
            break #unfinished_parses is empty ->break
